...
function normalizeCloudApiRoot(raw) {
...
async function validateManifestShape(manifestJson) {
...
async function resolveArtifactUri(opts, manifest, tarballAbsolute) {
    if (opts.artifactUri) {
    ...
    const region = opts.s3Region ?? process.env.ESCALATED_S3_REGION ?? process.env.AWS_REGION ?? process.env.AWS_DEFAULT_REGION;
    const endpoint = opts.s3Endpoint ?? process.env.ESCALATED_S3_ENDPOINT ?? process.env.AWS_ENDPOINT_URL ?? undefined;
    if (!bucket || !region) {
        throw new TypeError('S3 uploads require ESCALATED_S3_BUCKET (or AWS_S3_BUCKET) and ESCALATED_S3_REGION (or AWS_REGION), or provide --artifact-uri.');
    }
    if (!publicBase.startsWith('https://')) {
        throw new TypeError('After uploading you must expose an https:// URL. Set ESCALATED_ARTIFACT_PUBLIC_BASE or pass --artifact-public-base.');
    }
    let prefix = opts.s3Prefix ?? process.env.ESCALATED_S3_PREFIX ?? `marketplace-plugins/${opts.publisherSlug}/${manifest.slug}/`;
    prefix = prefix.replace(/\\/g, '/');
    const normalizedPrefix = prefix.endsWith('/') ? prefix : `${prefix}/`;
    const safeName = basename(tarballAbsolute).replace(/[^a-z0-9._-]+/gi, '-') || `${manifest.slug}-${manifest.version}.tar.gz`;
...
export async function publishPlugin(opts) {
    ...
    const manifestRaw = JSON.parse(await readFile(resolve(opts.manifestPath), 'utf8'));
