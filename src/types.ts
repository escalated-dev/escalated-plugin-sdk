...
export type EndpointDefinition =
    | ((ctx: PluginContext, req: EndpointRequest) => Promise<unknown>) // Shorthand
    | EndpointHandler; // With capability
...
// ─── Plugin Manifest (returned to host) ───
...
export interface PluginManifest {
    ...
    pages: PageDeclaration[];
    components: ComponentDeclaration[];
    widgets: Array<Omit<WidgetDeclaration, 'badge'> & { hasBadge: boolean }>;
    actionHooks: string[];
    filterHooks: Array<{ hook: string; priority: number }>;
    endpoints: Array<{ method: string; path: string; capability?: string }>;
    webhooks: Array<{ method: string; path: string }>;
